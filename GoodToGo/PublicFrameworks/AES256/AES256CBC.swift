...
public struct Random {
    public static func generate(_ upperBound: Int) -> Int { return Int(arc4random_uniform(UInt32(upperBound))) }
}
...
final public class AES256CBC {
    ...
    static let ivLen         : Int = 16
    static let baseEnconding : String.Encoding = .utf8
    ...
    public class func encryptString(_ str: String, password: String) -> String? {
        if !str.isEmpty && password.count == 32 {
            let iv  = randomText(AES256CBC.ivLen)
            let key = password
    ...
    public class func decryptString(_ str: String, password: String) -> String? {
        if str.count > 16 && password.count == 32 {
            guard let decryptedString = try? aesDecrypt(str, key:password) else {
    ...
    fileprivate enum CharType: Int {
        ...
        func randomCharacter() -> UInt8 {
            switch self {
            case .LowerCase: return UInt8(Random.generate(26)) + 97
            case .UpperCase: return UInt8(Random.generate(26)) + 65
            case .Digit: return UInt8(Random.generate(10)) + 48
    ...
    fileprivate class func aesEncrypt(_ str: String, key: String, iv: String) throws -> String {
        let keyData = key.data(using: AES256CBC.baseEnconding)!
        let ivData  = iv.data(using: AES256CBC.baseEnconding)!
        let data    = str.data(using: AES256CBC.baseEnconding)!
        let enc     = try Data(bytes: AESCipher(key: keyData.bytes, iv: ivData.bytes).encrypt(bytes: data.bytes))
        
        let newdata = enc + ivData
        #if os(Linux)
            return Base64.encode([UInt8](enc))
        #else
            return newdata.base64EncodedString(options: [])
        #endif
    }

    /// returns decrypted string, IV must be 16 chars long
    fileprivate class func aesDecrypt(_ str: String, key: String) throws -> String {
        
        let secretData1     = Data(base64Encoded: str, options: Data.Base64DecodingOptions.ignoreUnknownCharacters)
        let secretData2     = [UInt8](secretData1!)
        let ivDataArray     = secretData2.suffix(AES256CBC.ivLen)
        let secretDataArray = secretData2.prefix(secretData2.count-AES256CBC.ivLen)
        
        let ivData     = [UInt8](ivDataArray)
        let secretData = [UInt8](secretDataArray)
        
        let keyData = key.data(using: AES256CBC.baseEnconding)!
        let dec     = try Data(bytes: AESCipher(key: keyData.bytes, iv: ivData).decrypt(bytes: secretData))
        guard let decryptStr = String(data: dec, encoding: AES256CBC.baseEnconding) else {
...
extension AESCipher {
    ...
    convenience public init(key: String, iv: String) throws {
        guard let kkey = key.data(using: AES256CBC.baseEnconding, allowLossyConversion: false)?.bytes,
            // swiftlint:disable conditional_binding_cascade
            let iiv = iv.data(using: AES256CBC.baseEnconding, allowLossyConversion: false)?.bytes else {
                // swiftlint:disable conditional_binding_cascade
