...
extension Presenter {
    class BlissDetails_Presenter: BasePresenterMVP {
        ...
        var viewModel: VM.BlissDetails_ViewModel? { didSet { DevTools.Log.log(appCode: .vmChanged); viewModelChanged() } }
    }
}
    ...
    func userDidPretendToShareByEmail() {
        ...
        blissQuestions_UseCase.shareQuestionBy(email: "www.meumail@gmail.com", url: linkToShare, checkHealth: true) { [weak self] (result) in
            ...
            case .success(let some):
                if some.success {
                    self.genericView?.displayMessage(Messages.Bliss.sharedWithSuccess, type: .success)
                } else {
        ...
        rxObservable_DoVote()
            ...
            .subscribe(
                onNext: { [weak self] _ in
                    guard let self = self else { DevTools.Log.log(appCode: .referenceLost); return }
                    self.genericView?.setActivityState(false)
                    self.genericView?.displayMessage(Messages.success.localised, type: .success)
                },
...
extension P.BlissDetails_Presenter: BasePresenterVMPProtocol {
    func view_deinit() {
...
extension P.BlissDetails_Presenter {
    ...
    private func viewModelChanged() {
        guard viewModel != nil, viewModel?.question != nil else {
            DevTools.Log.log(appCode: AppCodes.notPredicted)
            return
        }
        view.set(title: (viewModel!.question!.question.description))
        view.viewNeedsToDisplay(list: viewModel!.question!.choices)
        downloadImage(imageURL: viewModel!.question!.imageURL, onFail: Images.notFound.image) { [weak self] (image) in
        ...
        if let data = blissGeneric_UseCase.screenHaveDataToHandle(screen: V.BlissDetails_View.className) {
            ...
            if key == AppConstants.Bliss.DeepLinks.questionId {
                 if let someInt = Int(value) {
                    DevTools.Log.log("Handling data!")
    ...
    func rxSetup() {
        ...
        blissGeneric_UseCase.rxPublishRelayApplicationDidReceivedData.asObservable().bind { [weak self] (_) in
