...
extension I {
    class ___VARIABLE_sceneName___Interactor: BaseInteractorWithWorkers, ___VARIABLE_sceneName___DataStoreProtocol {
...
extension I.___VARIABLE_sceneName___Interactor: ___VARIABLE_sceneName___BusinessLogicProtocol {
    ...
    func requestSomeStuff(request: VM.___VARIABLE_sceneName___.SomeStuff.Request) {
        ...
        presenter?.presentLoading(response: LoadingModel(isLoading: true, message: ""))
        DispatchQueue.executeWithDelay(delay: 1) { [weak self] in
            ...
            self?.presenter?.presentLoading(response: LoadingModel(isLoading: false, message: ""))
        }
    }
