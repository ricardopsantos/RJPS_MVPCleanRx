...
// typealias? Why?
//
// 1 : When using RJSLib on other projects, instead of using `DataModelEntetie.PersistentSimpleCacheWithTTL`, we can use `RJS_PersistentCacheWithTTL` which can be more elegant and short to use
// 2: When using RJSLib, we can type `RJS_` and the Xcode auto-complete feature will suggest only thing inside RJSLib
// 3 : if one day the module `DataModelEntetie.PersistentSimpleCacheWithTTL` changes name for something like `DataModelEntetie.PersistentSimpleCacheWithSmallTTL`,
...
public typealias RJS_HotCache               = RJS_LiveCache
public typealias RJS_ColdCache              = RJS_PersistentCacheWithTTL
...
public typealias RJS_CacheStrategy    = RJSLib.Storages.CachePersistant.CacheStrategy
