...
struct RJSLibNetworkClientErrorsManager {
    ...
    static func logError(response: HTTPURLResponse, request: RJSLibNetworkClientRequest_Protocol) {
        let successRange = 200...299
        guard !successRange.contains(response.statusCode) else { return }
        
        func log(request: RJSLibNetworkClientRequest_Protocol, status: Int, info: String="") {
            let url = request.urlRequest.url!.absoluteURL
            RJS_Logs.error("# Request [\(url)] failed with status code \(status)] \(info)")
        }
