    ...
    public struct PermissionsManager {
        ...
        public static func checkPHPhotoLibrary(completition:@escaping (Bool?) -> Void) {
            ...
            case .authorized:
                completition(true)
            case .notDetermined:
                RJS_Logs.DLog("Asking permision")
                completition(nil)
            ...
            case .restricted:
                RJS_Logs.DLog("\(_notGranted) : User do not have access to photo album.")
                completition(false)
            case .denied:
                RJS_Logs.DLog("\(_notGranted) : User has denied the permission.")
                completition(false)
            @unknown default:
                RJS_Logs.DLog("Unknow.")
                completition(false)
            }
        }
    }
