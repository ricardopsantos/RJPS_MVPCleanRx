...
struct NetworkClientErrorsManager {
    ...
    static func logError(response: HTTPURLResponse, request: RJSLibWebAPIRequest_Protocol) {
        let successRange = 200...299
        guard !successRange.contains(response.statusCode) else { return }
        
        func log(request: RJSLibWebAPIRequest_Protocol, status: Int, info: String="") {
            let url = request.urlRequest.url!.absoluteURL
            RJS_Logs.DLogError("# Request [\(url)] failed with status code \(status)] \(info)")
        }
