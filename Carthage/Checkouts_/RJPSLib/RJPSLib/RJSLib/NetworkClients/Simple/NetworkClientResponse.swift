...
public enum NetworkClientResponseType {
    case json
    case csv
}

public struct NetworkClientResponse<T: Codable> {
    public let entity: T
    public let httpUrlResponse: HTTPURLResponse?
    public let data: Data?
    public let responseType: NetworkClientResponseType
    public init(data: Data?, httpUrlResponse: HTTPURLResponse?, responseType: NetworkClientResponseType) throws {
        do {
            switch responseType {
            case .json: self.entity = try JSONDecoder().decode(T.self, from: data!)
            case .csv : self.entity = try NetworkClientResponse.parseCSV(data: data!)
            }
