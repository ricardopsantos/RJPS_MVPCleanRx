    ...
    class HerokuServer {
        ...
        private func serverURL(url:@escaping (String) -> Void) {
            func testAndSwap(server1: String, server2: String) {
                RJS_SimpleNetworkClient.jsonFrom(urlString: serverA+"/test") { (some, success) in
                    if some != nil && success {
                        url(server1)
                    } else {
                        url(server2)
        ...
        public func sendMessageToServer(_ message: String, type: String="Info") {
            serverURL { [weak self] (url) in
                ...
                RJS_SimpleNetworkClient.jsonFrom(urlString: finalURL, completion: { (_, _) in })
            }
        }
        ...
        public func getValueFromServer(key: String, completition:@escaping (AnyObject?) -> Void) {
            ...
            serverURL { [weak self] (url) in
                ...
                RJS_SimpleNetworkClient.jsonFrom(urlString: finalURL, completion: { (some, success) in
